...
def generate_launch_description():
    ...
    cmdVelTest_cfg_dir = LaunchConfiguration("cmdVelTest_cfg_dir")
    ...
    declare_cmdVelTest_cfg_dir = DeclareLaunchArgument(
        "cmdVelTest_cfg_dir",
        default_value=PathJoinSubstitution([communication_dir, "config", "cmd_vel_test.yaml"]),
        description="Path to the cmd_vel_test config file",
    )
    ...
    start_cmdVelTest_node = Node(
        package="communication",
        executable="cmd_vel_test_node",
        namespace=namespace,
        parameters=[cmdVelTest_cfg_dir],
        output="screen",
    )
    ...
    ld.add_action(declare_cmdVelTest_cfg_dir)
    ld.add_action(start_communication_node)
    ld.add_action(start_cmdVelTest_node)
